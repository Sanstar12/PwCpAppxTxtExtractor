...
BOT_TOKEN = os.environ.get("BOT_TOKEN", "8123711815:AAH9Dxnyn86kHGm1Ruv97kp4DRYNjT9Rqw8")
auth_users = [123456]  # Replace with your actual auth_users list or import from config
...
def run_flask():
    app.run(host="0.0.0.0", port=8080)  # Use 8080 for Koyeb/Render

# ThreadPool for async tasks
THREADPOOL = ThreadPoolExecutor(max_workers=50)  # Reduced workers for efficiency
...
async def fetch_cpwp_signed_url(url_val: str, name: str, session: aiohttp.ClientSession, headers: dict) -> str | None:
    MAX_RETRIES = 3
    for attempt in range(MAX_RETRIES):
        ...
        try:
            ...
            logging.error(f"Attempt {attempt + 1} failed fetching signed URL for {name}: {e}")
        if attempt < MAX_RETRIES - 1:
            await asyncio.sleep(2 ** attempt)
    logging.error(f"Failed to fetch signed URL for {name} after {MAX_RETRIES} attempts.")
    return None

async def process_cpwp_url(url_val: str, name: str, session: aiohttp.ClientSession, headers: dict) -> str | None:
    try:
...
async def get_cpwp_course_content(session: aiohttp.ClientSession, headers: dict, batch_token: str, folder_id: int = 0, limit: int = 9999999999, retry_count: int = 0) -> tuple[list[str], int, int, int]:
    MAX_RETRIES = 3
    ...
    try:
        ...
        async with session.get(content_api, params=params, headers=headers) as res:
            ...
            contents: list[dict] = res_json['data']
        ...
        if retry_count < MAX_RETRIES:
            ...
            logging.error(f"Failed to retrieve folder {folder_id} after {MAX_RETRIES} retries: {e}")
            return [], 0, 0, 0
...
async def process_cpwp(bot: Client, m: Message, user_id: int):
    ...
    CONNECTOR = aiohttp.TCPConnector(limit=100, loop=loop)  # Reduced limit
    async with aiohttp.ClientSession(connector=CONNECTOR, loop=loop) as session:
        try:
                                ...
                                async with session.get(f"https://api.classplusapp.com/v2/course/preview/org/info", params=params, headers=batch_headers) as response:
                                    if response.status == 200:
                                        ...
                                        if course_content:
                                            ...
                                            formatted_time = f"{minutes} minutes {seconds} seconds" if minutes > 0 else f"{seconds} seconds" if seconds >= 1 else f"{response_time:.2f} seconds"
            ...
            logging.exception(f"Error in process_cpwp: {e}")
            await editable.edit(f"**Error: {e}**")
        finally:
            await session.close()
